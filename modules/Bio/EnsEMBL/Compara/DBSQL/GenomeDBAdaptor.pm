...
sub fetch_by_name_assembly {
  ...
  # kb8:
  #
  # For some odd reason compara strips out the _ in my organism names, so we have to
  # add then again ourself.
  #
...
#  my ($id) = $sth->fetchrow_array();
  ...
  if (!defined $id) {
    die("No GenomeDB with this name [$name] and assembly [$assembly or --undef--)]");
  }
  $sth->finish;
...
sub fetch_by_Slice {
  ...
  print STDERR "Fetching by a core_dba\n";
